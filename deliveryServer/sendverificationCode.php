...
function sendmail($email, $verficationCode)
{
    ...
    try {
        ...
        $mail->Username = 'xxxxxx@qq.com';                // SMTP 用户名  即邮箱的用户名
        $mail->Password = 'xxxxxxxxxxxxxxxx';             // SMTP 密码  部分邮箱是授权码(例如163邮箱)
        $mail->SMTPSecure = 'tls';                    // 允许 TLS 或者ssl协议
        $mail->Port = 587;                            // 服务器端口 25 或者465 具体要看邮箱服务器支持

        $mail->setFrom('xxxxxx@qq.com', 'Mailer');  //发件人
        $mail->addAddress($email, 'Joe');  // 收件人
        //$mail->addAddress('ellen@example.com');  // 可添加多个收件人
        $mail->addReplyTo('xxxxxx@qq.com', 'info'); //回复的时候回复给哪个邮箱 建议和发件人一致
